...
func (p *LiveKitProcess) generateConfig() (string, error) {
	...
	rtcStart, rtcEnd, err := config.EffectiveLiveKitRTCPortRange(p.cfg)
	if err != nil {
		return "", err
	}
	useExternalIP := config.EffectiveLiveKitUseExternalIP(p.cfg)
...
logging:
  level: info
`, rtcStart, rtcEnd, useExternalIP, nodeIPLine, p.cfg.LiveKitAPIKey, p.cfg.LiveKitAPISecret)
